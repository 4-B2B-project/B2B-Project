...
function searchUser() {
	...
	.then(result => {
		...
		if(result > 0) {
			document.getElementById('srchPwDiv').style.display = 'none';
			document.getElementById('changePwDiv').style.display = 'flex';
			
			$("#memberId").val(inputId.value);
			$("#memberEmail").val(inputEmail.value);
...
if (changeSubmit != null) {
  // 제출 되었을 때
  changeSubmit.addEventListener("submit", e => {
