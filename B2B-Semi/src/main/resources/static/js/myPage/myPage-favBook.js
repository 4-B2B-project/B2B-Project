...
bookInfoRows.forEach(row => {
  ...
  row.addEventListener('click', () => {
    ...
    // Split and create genre badges only if bookGenres is not empty

    // bookGenres가 null 또는 빈 문자열이 아니면 분리하여 배지 생성
    if (bookGenres && bookGenres !== "null") {
      bookGenres.split(',').forEach(genre => {
        // 태그 생성 및 class, text입히기
        const genreBadge = document.createElement('span');
        genreBadge.className = 'badge bg-primary me-2';
        genreBadge.textContent = genre; // Trim extra spaces
        // 장르 태그 안에 자식으로 추가
        genreContainer.appendChild(genreBadge);
      });
    } else {
      genreContainer.innerHTML = '<span class="text-muted">장르 정보가 없습니다.</span>';
    }
