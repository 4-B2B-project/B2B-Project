...
if (deleteBtn != null) {
  deleteBtn.addEventListener("click", () => {
    ...
    // 삭제 후 현재 페이지의 게시글 목록을 다시 불러옵니다.
    form.addEventListener("submit", () => {
      // 삭제가 완료된 후 목록 페이지로 리다이렉트하여 페이지 갱신
    window.location.href = `/myPage/boardList?cp=${cp}&searchType=${type}&searchInput=${encodedSearchInput}`;
    });
  });
}
